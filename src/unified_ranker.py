"""
XBRL Minimal Repair - Single Unified Ranker
Addressing Codex Round 4 recommendation: one globally comparable ranking model
Key insight: Two independent rerankers have incomparable probabilities (fatal trade-off)
Solution: Single XGBoost ranker over ALL candidates, with role features and interactions
"""
...
def build_unified_features(task, candidate_ids, violation_scores):
    """Build unified features for ALL candidates (total + component compete directly)

    Key insight from Codex Round 4:
    - Two independent rerankers have incomparable probability outputs
    - Solution: Single ranker where total and component candidates compete
    """
    features = []
    ...
    # Compute global role distribution for this task
    total_residual_sum = sum(v["residual"] for v in task.violations)
    n_violations = len(task.violations)

    for fid in candidate_ids:
        residual_ratio = 0.0  # Initialize for each candidate
        fact = None
        for f in task.facts:
            if f["id"] == fid:
                fact = f
        ...
        if fact is None:
            features.append([0] * 38)  # 38 total features (no leakage)
            continue

        feat = []

        # === Role features (candidates compete on these) ===
        ...
        feat.append(1.0 if is_total_pos else 0.0)  # is_total
        feat.append(1.0 if is_component_pos else 0.0)  # is_component
        feat.append(1.0 if (is_total_pos or is_component_pos) else 0.0)  # is_involved

        # === Role × Residual interaction (key for discrimination) ===
        if is_total_pos:
            # Total-specific residual features
            for v in task.violations:
                if f"fact_{v['row']}_{v['total_col']}" == fid:
                    feat.append(v["residual"] / max_residual)  # total_residual
                    feat.append(abs(fact["value"] - v["expected_sum"]) / max_value)  # total_discrepancy

                    # Signed residual direction (positive/negative error)
                    signed_residual = (v["reported_total"] - v["expected_sum"]) / max_value
                    feat.append(signed_residual)

                    # Fix potential: if total is wrong, fixing it alone resolves violation
                    feat.append(1.0)  # total_fix_potential
                    break
            else:
            ...
            for v in task.violations:
                for j, _ in v.get("components", []):
                    if f"fact_{v['row']}_{j}" == fid:
                        ...
                        # Component's contribution to residual
                        feat.append(v["residual"] / max_residual / len(v["components"]))
                        break
                else:
                    continue
                break
        ...
        # === Violation features (shared by all candidates) ===
        ...
        # === Observable error pattern features (inference-time derivable) ===
        # NO LABEL LEAKAGE - derived from residual/value patterns, not error_type labels

        # Residual magnitude relative to value (can distinguish scale_10 vs value_shift)
        if is_total_pos:
            for v in task.violations:
                if f"fact_{v['row']}_{v['total_col']}" == fid:
                    residual_ratio = v["residual"] / (abs(fact["value"]) + 1)
                    feat.append(residual_ratio)
                    # Large residual ratio suggests scale/sign issue
        ...
        # Signed residual agreement (is value sign opposite to expected?)
        if is_total_pos:
            for v in task.violations:
                if f"fact_{v['row']}_{v['total_col']}" == fid:
                    # If reported total and expected sum have opposite signs
        ...
        # Value magnitude anomaly (is value much larger/smaller than expected?)
        if is_total_pos:
            for v in task.violations:
                if f"fact_{v['row']}_{v['total_col']}" == fid:
                    magnitude_ratio = abs(fact["value"]) / (abs(v["expected_sum"]) + 1)
                    feat.append(magnitude_ratio)
                    feat.append(1.0 if magnitude_ratio > 5 else 0.0)  # possible scale_10
                    break
            else:
                feat.extend([0.0, 0.0])
        elif is_component_pos:
                ...
                for j, _ in v.get("components", []):
                    if f"fact_{v['row']}_{j}" == fid:
                        other_sum = sum(task._get_cell_value(v["row"], j2) or 0
                                       ...
                                       if j2 != j)
                        expected_val = v["reported_total"] - other_sum
                        magnitude_ratio = abs(fact["value"]) / (abs(expected_val) + 1)
                        feat.append(magnitude_ratio)
        ...
        # === Role × Observable pattern interactions ===
        feat.append(1.0 if is_total_pos and residual_ratio > 5 else 0.0)  # total_large_residual
        feat.append(1.0 if is_component_pos and residual_ratio > 5 else 0.0)  # comp_large_residual

        # === Global context ===
        feat.append(n_violations / 5)
        feat.append(total_residual_sum / (max_residual * n_violations) if n_violations > 0 else 0)
        if std_val > 0:
            feat.append(abs(abs(fact["value"]) - mean_val) / std_val)
        else:
            feat.append(0.0)

        feat.append(1.0)  # bias term
...
class UnifiedRanker:
    """Single unified XGBoost ranker over ALL candidates

    Key design decisions (from Codex Round 4):
    1. Train with candidate-set grouping: total and component compete directly
    2. Role features and role × error interactions for discrimination
    3. Single probability output - no calibration mismatch
    """
    ...
    def train(self, train_tasks):
        """Train unified ranker with candidate-set grouping"""
        X_all = []
        y_all = []
        groups = []  # For potential group-aware training
        group_sizes = []
        ...
        print(f"  Positive samples: {sum(y_all)}, Negative: {len(y_all) - sum(y_all)}")
        print(f"  Total-target tasks: {sum(1 for t in train_tasks if t.target_is_total)}")
        print(f"  Component-target tasks: {sum(1 for t in train_tasks if not t.target_is_total)}")
    ...
    def localize(self, task):
        """Rank ALL candidates by unified probability scores"""
        candidates, violation_scores = self._get_candidates(task)
...
def run_unified_ranker():
    """Run unified ranker experiment"""
    print("=" * 60)
    print("XBRL Minimal Repair - Unified Ranker")
    print("Addressing Codex Round 4: Single globally comparable model")
    print("=" * 60)
    ...
    if len(test_total) > 0:
        ...
        top1 = sum(r["hit_1"] for r in total_results) / len(total_results)
        top3 = sum(r["hit_3"] for r in total_results) / len(total_results)
        print(f"\nTotal-target tasks (n={len(test_total)}):")
        print(f"  Top-1: {top1:.2%}")
        print(f"  Top-3: {top3:.2%}")

    # Evaluate component-target
    if len(test_component) > 0:
        ...
        top1 = sum(r["hit_1"] for r in comp_results) / len(comp_results)
        top3 = sum(r["hit_3"] for r in comp_results) / len(comp_results)
    ...
    print(f"  Overall Top-1: {top1:.2%}, 95% CI=[{ci_lower:.2%}, {ci_upper:.2%}]")

    # Role-specific CI
    if len(test_total) >= 3:
        total_hits = [r["hit_1"] for r in total_results]
        bootstrap_total = []
        for _ in range(500):
            indices = np.random.choice(len(total_hits), size=len(total_hits), replace=True)
            bootstrap_total.append(np.mean([total_hits[i] for i in indices]))
        print(f"  Total-target Top-1: 95% CI=[{np.percentile(bootstrap_total, 2.5):.2%}, {np.percentile(bootstrap_total, 97.5):.2%}]")

    if len(test_component) >= 3:
        comp_hits = [r["hit_1"] for r in comp_results]
        bootstrap_comp = []
        for _ in range(500):
            indices = np.random.choice(len(comp_hits), size=len(comp_hits), replace=True)
            bootstrap_comp.append(np.mean([comp_hits[i] for i in indices]))
        print(f"  Component-target Top-1: 95% CI=[{np.percentile(bootstrap_comp, 2.5):.2%}, {np.percentile(bootstrap_comp, 97.5):.2%}]")

    # ============================================================
    # Save results
    # ============================================================
    results = {
        "experiment": "unified_ranker",
        "overall": {"top1": top1, "top3": top3, "mrr": mrr, "ci_lower": ci_lower, "ci_upper": ci_upper},
        "total_target": {"top1": sum(r["hit_1"] for r in total_results) / len(total_results) if total_results else 0,
                         "n": len(test_total)},
        "component_target": {"top1": sum(r["hit_1"] for r in comp_results) / len(comp_results) if comp_results else 0,
                             "n": len(test_component)},
        "test_size": len(test)
    }

    with open("data/benchmark/unified_ranker_results.json", "w") as f:
        json.dump(results, f, indent=2)

    print("\nSaved to data/benchmark/unified_ranker_results.json")
...
if __name__ == "__main__":
    run_unified_ranker()
    print("\n" + "=" * 60)
    print("Unified Ranker Experiment Complete")
    print("=" * 60)